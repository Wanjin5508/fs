...
在这一章节, 第四章提到的被不同研究者分析并评估的多种基于AI的方法和实践被总结. 本研究在第一章提出的第一个研究问题会在本节被回答。按照算法交易发展的时间顺序, 本研究首先分析了基于统计学和数学的方法。一个典型的案例是automated exit and re-entry strategy, 这个策略展示出了算法交易基于市场现状进行自动化的投资决策的特点。研究者们将AI技术融合到传统的算法交易中, 这一过程的本质是借助AI技术的辅助优化对特定金融市场风险的评估并提高投资决策的执行效率。
...
为了回答在第一章提出的第二个研究问题, 本研究在系统性文献检索的结果的基础上结合作者自身作为AI应用开发者的经验, 对AI给算法交易带来的机遇和挑战进行了深入的分析和探讨。因此本章节的目的在于, 实现不同研究者对于AI对算法交易的影响的观点的整合。拥有不同研究重点的研究者对于AI的观点在这一节相互碰撞, 读者们则可以站在巨人的肩上掌握算法交易中AI技术的最新影响。
